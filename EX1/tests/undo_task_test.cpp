...
int main() {
    ...
    const tools::Optional<tfm::Task> undo_result = task_manager.undoLastProcessedTask();
    assert(undo_result == tools::nullopt);

    for (short i = 0; i < 2; i++) {
        task_manager.processNextTasks();

        const tools::Optional<tfm::Task> undo_result_2 = task_manager.undoLastProcessedTask();
